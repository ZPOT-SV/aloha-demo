...
export default function StoreSelectionModal({
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/55 px-4 py-4 backdrop-blur-sm sm:px-6 sm:py-8">
      <div className="flex min-h-full items-start justify-center sm:items-center">
        <div className="flex max-h-[calc(100dvh-2rem)] w-full max-w-3xl flex-col rounded-[2rem] border border-brand/10 bg-white p-5 shadow-2xl shadow-brand/20 sm:max-h-[calc(100dvh-4rem)] sm:p-6">
          ...
          <div className="mt-6 flex shrink-0 flex-col-reverse gap-3 border-t border-brand/10 pt-4 sm:flex-row sm:flex-wrap sm:items-center sm:justify-end sm:gap-4">
            <button
              className="w-full text-center text-sm font-bold text-brand transition hover:text-deep-cta sm:w-auto"
              onClick={onCancel}
              type="button"
            >
              Cancelar
            ...
            <button
              className="w-full rounded-full bg-cta px-5 py-3 text-sm font-bold text-white shadow-lg shadow-cta/20 transition hover:bg-deep-cta disabled:cursor-not-allowed disabled:bg-slate-300 sm:w-auto"
